...
static int getAndInitPage(
  ...
  //Dohwan
 /*
  if(!is_ovflpage(pgno)&& pgno>btreePagecount(pBt)){
  	rc = SQLITE_CORRUPT_BKPT;
...
static int getOverflowPage(
  ...
  */
  ...
  if( rc==SQLITE_OK ){
	  ...
	  //end
	  //Dohwan-mod check for flags debug
	  int flagToPass = (ppPage==0) ? PAGER_GET_READONLY : PAGER_GET_OVERFLOW;
	  //fprintf("[DEBUG] getOverflowPage - ovfl_pgno: %u, flags: %d, PAGER_GET_OVERFLOW: %d\n", 
			              //  ovfl, flagToPass, PAGER_GET_OVERFLOW);
	  //end
	  //vanilla
...
static int accessPayload(
  ...
  //Dohwan confirm firstpgno of ovflpage
  Pgno firstOvfl = get4byte(&aPayload[pCur->info.nLocal]);
  //fprintf(stderr,"firstpgno : %d", firstOvfl);
  //end
  ...
  if( rc==SQLITE_OK && amt>0 ){
    ...
    */
    if( (pCur->curFlags & BTCF_ValidOvfl)==0 ){
      ...
      //Dohwan
      //fprintf(stderr,"npayload size : %d ovfl cnt : %d\n",pCur->info.nPayload,nOvfl);
      //end
      if( pCur->aOverflow==0
      ...
      //Dohwan-single request for ovflpage
      if(pBt->pPager->tempFile != 1){
        //sqlite3_file *fd = pCur->pBt->pPager->fd;
        if(nOvfl > 0){
	  u32 pgno = convert_ovfl_pgno(firstOvfl);
          const i64 off = ((i64)pgno) * 4096;
          const int nBytes = nOvfl * 4096;
          u8 *buf = (u8*)sqlite3Malloc(nBytes);
          if(!buf) return SQLITE_NOMEM_BKPT;
	  //Dohwan have to fix
	  lseek(pBt->pPager->ovflFd, off, SEEK_SET);
	  read(pBt->pPager->ovflFd, buf, nBytes);
	  //Dohwan confirm
          //int rc = sqlite3OsRead(fd, buf, nBytes, off);
          sqlite3_free(buf);
          //if(rc!=SQLITE_OK){ return rc; }
        }
     }
	...
	//	fprintf(stderr, "point5");
	  ...
	  //end
          if( rc==SQLITE_OK ){
            //Dohwan-mod pagerGet ok
	    //fprintf(stderr,"pagerGet-ovfl ok");
  ...
  if( n>0 ){
    ...
    */
    do {
      ...
      //Dohwan
      if(!is_ovflpage(iTrunk) &&  (iTrunk>mxPage || nSearch++ > n) )
      {
	...
	//Dohwan
	if(!is_ovflpage(iPage) && (iPage>mxPage || iPage<2) )
	{
          rc = SQLITE_CORRUPT_PGNO(iTrunk);
          goto end_allocate_page;
        }
...
static int freePage2(BtShared *pBt, MemPage *pMemPage, Pgno iPage){
  ...
  //Dohwan
  if(!is_ovflpage(iPage) && (iPage<2 || iPage>pBt->nPage) )
  {
    return SQLITE_CORRUPT_BKPT;
  }
  if( pMemPage ){
  ...
  */
  if( nFree!=0 ){
    ...
    if( !is_ovflpage(iTrunk) && iTrunk>btreePagecount(pBt) )
    {
      rc = SQLITE_CORRUPT_BKPT;
      goto freepage_out;
    }
...
static SQLITE_NOINLINE int clearCellOverflow(
  ...
  while( nOvfl-- ){
    ...
    if(!is_ovflpage(ovflPgno) && (ovflPgno<2 || ovflPgno>btreePagecount(pBt)) )
    {
  ...
  while( 1 ){
    ...
    ** is still writeable */
    ...
    if( spaceLeft==0 ){
      ...
      }else{rc = allocateBtreePage(pBt, &pOvfl, &pgnoOvfl, pgnoOvfl, 0);}	      
      //end
#ifndef SQLITE_OMIT_AUTOVACUUM
...
int sqlite3BtreeCreateTable(Btree *p, Pgno *piTable, int flags){
  ...
  sqlite3BtreeEnter(p);
  //Dohwan
  if (p->pBt->openFlags ==  (BTREE_OMIT_JOURNAL | BTREE_SINGLE)) {
  	tempBtreeFlag = 1;
  }
  //end

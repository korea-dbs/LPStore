...
static int pagerWalFrames(
  ...
  if( isCommit ){
    ...
    ** list here. */
	...
	//if( p->pgno<=nTruncate)    
      //Dohwan
      if( p->pgno<=nTruncate|| is_ovflpage(p->pgno) )
      {
        ppNext = &p->pDirty;
        nList++;
      }
    ...
    */
    if( rc==SQLITE_OK ){
      ...
      if (access(zOvflFilename, F_OK) == 0) {
	      ...
	      ovflPageCnt = pPager->ovflPageCnt;

	      //fprintf(stderr, "[INFO] open existing overflow file: %s fd: %d file size: %u\n", zOvflFilename, pPager->ovflFd, filesz);
#ifdef UNIV_HASH
...
static int getPageNormal(
  ...
  if( pBase==0 ){
    pPg = 0;
    //Dohwan fetch stress getnormal
    fprintf(stderr,"pachefetstress\n");
...
/*
  ...
  if( rc==SQLITE_OK ){
	 libsql_wal *pWal = (libsql_wal*)sqlite3MallocZero(sizeof(libsql_wal));
	 if( !pWal ){
	 	rc = SQLITE_NOMEM;
	 }else{
	//	rc = sqlite3WalOpen((wal_manager_impl *)&pPager->wal_manager->ref,
      	//		pPager->pVfs,pPager->fd,pPager->ovflFd,pPager->exclusiveMode,
    	//		pPager->journalSizeLimit,pPager->zWal,pWal);
	        rc = sqlite3WalOpen((wal_manager_impl *)&pPager->wal_manager->ref.pData,
			                        pPager->pVfs,pPager->fd,pPager->ovflFd,pPager->exclusiveMode,
					                        pPager->journalSizeLimit,pPager->zFilename,pWal);	
		if( rc==SQLITE_OK ){
			pPager->wal = pWal;
		}else{
			sqlite3_free(pWal);

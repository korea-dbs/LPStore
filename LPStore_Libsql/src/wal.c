...
//Dohwan
/*
static void sqlite3WalEncodeOvflPgno(
		Wal *pWal,
		u32 ovflPgno,
		u8 *aFrame){
...
static void sqlite3WalDecodeOvflPgno(
		Wal *pWal,
		u32 *ovflPgno,
		u8 *aFrame){
	assert( WAL_FRAME_HDRSIZE==24 );
...
static int walCheckpoint(
  ...
  if( pInfo->nBackfill<pWal->hdr.mxFrame ){
    ...
    */
	...
	//Dohwan
	if( !is_ovflpage(iDbpage)&& (iFrame<=nBackfill || iFrame>mxSafeFrame || iDbpage>mxPage ))
	{
          continue;
        }
        iOffset = walFrameOffset(iFrame, szPage) + WAL_FRAME_HDRSIZE;
        /* testcase( IS_BIG_INT(iOffset) ); // requires a 4GiB WAL file */
	//Dohwan-mod-checkpoint
	...
	if (is_ovflpage(iDbpage)){
		...
		u32 ovfl_idx = convert_ovfl_pgno(iDbpage);
		i64 ovflOffset = (i64)ovfl_idx * szPage;
		/*
		if( lseek(pWal->pOvflFd, ovflOffset, SEEK_SET) != (off_t)-1 ){
			fprintf("ovfl_idx : %d",ovfl_idx);
			ssize_t written = write(pWal->pOvflFd, zBuf, szPage);
			if( written == (ssize_t)szPage ){
				ovfl_flag = 1;
			}else{
...
static int walWriteToLog(
  ...
  int rc;
  //Dohwan walWriteToLog
  //fprintf(stderr, "walwritetolog\n");
  //end
...
const libsql_wal_manager sqlite3_wal_manager = { 
    .pData = NULL,
   //Dohwan-comment dont have to fix parameter 

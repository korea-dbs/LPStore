      ...
      */
      ...
      if( nCell<=pPage->max1bytePayload ){
        ...
        ** case this happens.  */
        ...
        if( nCell<2 || nCell/pCur->pBt->usableSize> (pCur->pBt->nPage + pCur->pBt->pPager->ovflPageCnt) ){
...
static int allocateBtreePage(
  ...
  ** stores the total number of pages on the freelist. */
  n = get4byte(&pPage1->aData[36]);
  testcase( n==mxPage-1 );
  if( n>=mxPage + pBt->pPager->ovflPageCnt && n>=mxPage ){
    return SQLITE_CORRUPT_BKPT;
  }
  if( n>0 ){
    /* There are pages on the freelist.  Reuse one of those pages. */
...
static SQLITE_NOINLINE int clearCellOverflow(
  ...
  while( nOvfl-- ){
    Pgno iNext = 0;
    MemPage *pOvfl = 0;
	// (jhpark): bypass **separated** overflow pages
      ...
      */
      ...
      // (jhpark): allocate new page
	      ...
	      if (rc != SQLITE_OK) {
		   ...
		   // here, we create/allocate overflow page for temp table
		   // this allocates and deallocates overflow page within memory
...
int sqlite3BtreeCreateTable(Btree *p, Pgno *piTable, int flags){
  int rc;
  sqlite3BtreeEnter(p);
  // (jhpark): set flag if creaet temp table!
